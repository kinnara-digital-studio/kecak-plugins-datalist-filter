<div>
    ...
    <div style="float: left;padding-right: 10px;">
        <input id="${name}-from" name="${name}-from" type="text" size="${properties.size!}" value="${valueFrom!?html}" class="${elementParamName!} datetimepicker" placeholder="From : ${label}" readonly/>
    </div>

    <#if (singleValue!'false') != 'true' >
        <div style="float: left;padding-right: 10px;">
            <input id="${name}-to" name="${name}-to" type="text" size="${properties.size!}" value="${valueTo!?html}" class="${elementParamName!} datetimepicker" placeholder="To : ${label}" readonly/>
        </div>
    </#if>
